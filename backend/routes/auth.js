...
router.post('/login', async (req, res) => {
	...
	if (!email || !password)
		return res.status(400).json({ message: 'Email und Passwort benötigt' });
...
router.post('/register', async (req, res) => {
	...
	try {
		...
		const existing = await conn.query('SELECT * FROM account WHERE email = ?', [
			email,
		]);
		...
		const token = jwt.sign(
			{ id: result.insertId.toString(), email },
			JWT_SECRET,
			{
				expiresIn: '2h',
			}
